...
<body>
    <div style="margin-top: 20px" class="container">
        <h1>Find Device from a Student</h1>
        <h5 style="color:red;">Note* Records not found will display a table with no rows</h5>
        <!-- styling of the form for bootstrap https://getbootstrap.com/docs/4.5/components/forms/ -->
        <form action="search_r8.php" method="post">  <!--CHANGE ACTION-->
        <div class="form-group">
                <label for="name">Student Name (Must Exist in the system)</label> <!--CHANGE for= -->
                <input class="form-control" type="text" id="name" name="name"> <!--CHANGE id= and name=-->
            </div>
            <div class="form-group">
    ...
    <?php
        ...
        if (isset($_POST['Submit'])){
            ...
            //output table
            echo "Device issued for: ".$SName;
            
            $queryDevice  = mysqli_query($con,"SELECT DI.DName FROM DEVICE_ISSUED AS DI WHERE DI.DDeviceID IN (SELECT D.DDeviceID FROM DEVICE AS D WHERE D.SID IN (SELECT S.SID FROM STUDENT AS S WHERE S.SName = '$SName'))");
            ?>
